...
public class Customer
{
    ...
    public String htmlStatement()
    {
        ...
        for (Rental each : _rentals)
        {
            // Lógica VNew: Adiciona se é Blue-Ray ou não
            String blueray_status = each.getMovie().getPrice().is_blueray() ? " (Blue-Ray)" : "";

            result += "\t<li>" + each.getMovie().getTitle() + blueray_status + "\t" +  each.getMovie().getPrice().getRentalAmount(each.getDaysRented()) +"\n";
        }
        result += "</ul>\n";
